...
// import { Table, Card, Button, Dropdown, Form, Breadcrumb, Alert } from 'react-bootstrap';
// import { Link } from 'react-router-dom';
// import DatePicker from 'react-datepicker';
// import 'react-datepicker/dist/react-datepicker.css';
// import { useEmployeeDetails } from "../Componets/Employee";
...
// const getRandomPunchOutTime = () => {
//     const punchOutTime = new Date();
//     punchOutTime.setHours(15, 30, 0, 0);
...
//         { date: '2024-04-13', name: "Sinhala and Tamil New Year's Eve" },
//         { date: '2024-04-14', name: "Sinhala and Tamil New Year" },
...
//             profile: employee.profile,
//             punchDate: new Date(),
//             punchIn: new Date(new Date().setHours(8, 30)),
//             punchOut: getRandomPunchOutTime(),
//             department: employee.department,
//             isApproved: false,
//             leaveStatus: null,
//             lateCount: 0,
...
//             row.id === id ? { ...row, leaveStatus: 'Approved' } : row
//         );
//         setData(newData);
//     };
...
//         const newData = data.map(row =>
//             row.id === id ? { ...row, leaveStatus: 'Rejected' } : row
//         );
//         setData(newData);
//     };

//     const isLate = (punchIn) => punchIn.getHours() > 8 || (punchIn.getHours() === 8 && punchIn.getMinutes() > 30);

//     const checkHoliday = (date) => sriLankaHolidays.isHoliday(date);

//     useEffect(() => {
//         const newAlerts = data.filter(row => row.lateCount >= 3).map(row => `${row.profile} has been late 3 or more times this month.`);
//         setAlerts(newAlerts);
//     }, [data]);
...
//     const exportToCSV = () => {
//         const csvRows = [];
//         const headers = ['Employee Name', 'ID', 'Punch Date', 'Punch In', 'Punch Out', 'Department', 'Status', 'Working Hours', 'Worked Status', 'Leave Status', 'Late Count'];
//         csvRows.push(headers.join(','));
...
//                 <Alert key={index} variant="warning" className="mx-3">
//                     {alert}
//                 </Alert>
//             ))}
//             <Card className="no-border">
...
//                                 {[...new Set(data.map(row => row.department))].map(dept => (
...
//             Delete
//         </Dropdown.Item>
//         <Dropdown.Item onClick={() => handleApproveLeave(row.id)}>
//             Approve Leave
//         </Dropdown.Item>
//         <Dropdown.Item onClick={() => handleRejectLeave(row.id)}>
//             Reject Leave
//         </Dropdown.Item>
//     </Dropdown.Menu>
// </Dropdown>
...
const Att_DailyLog = () => {
    ...
    const handleSave = () => {
        const newData = [...data];
        newData[editIndex] = { ...editedRow };
        if (isLate(editedRow.punchIn)) {
            newData[editIndex].lateCount += 1;
        }
        setData(newData);
        setEditIndex(null);
    };
    ...
    const checkHoliday = (date) => sriLankaHolidays.some(holiday => holiday.date === date.toISOString().split('T')[0]);
                            ...
                            {filteredData.map((row, index) => {
                                ...
                                return (
                                    <tr key={row.id}>
                                        ...
                                        <td>{`${hours}h ${minutes}m`}</td>
                                        <td>{workedStatus}</td>
                                        <td>{row.leaveStatus || 'Not - Allowed'}</td>
                                        <td>{row.lateCount}</td>
                                        {/* <td>
                                            {editIndex === index ? (
                                                <>
